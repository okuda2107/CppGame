...
if(-not (Test-Path $(Join-Path $ExlibsDir "SDL2-2.32.0")) -eq $false) {
    Download-Lib "https://github.com/libsdl-org/SDL/releases/download/release-2.32.8/SDL2-devel-2.32.8-VC.zip" $ExlibsDir
}

if(-not (Test-Path $(Join-Path $ExlibsDir "SDL2_image-2.8.8")) -eq $false) {
...
if(-not (Test-Path $(Join-Path $ExlibsDir "SDL2_mixer-2.8.1")) -eq $false) {
...
if(-not (Test-Path $(Join-Path $ExlibsDir "Simple-OpenGL-Image-Library")) -eq $false) {
    git clone https://github.com/kbranigan/Simple-OpenGL-Image-Library.git $ExlibsDir
