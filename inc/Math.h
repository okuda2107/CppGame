...
public:
	...
	explicit Vector2(float x, float y) : x(x), y(y)//explicitにすると暗黙的な型変換がされなくなる　この書き方での初期化しか認めませんよってこと
	{}
	Vector2() : x(0.0f), y(0.0f)
	{}

	const friend Vector2 operator+ (const Vector2& a, const Vector2& b)
	{
	...
	const friend Vector2 operator* (const Vector2& vec, float scalor)//
	{
		Vector2 temp(vec.x * scalor, vec.y * scalor);
		return temp;
	}

	friend Vector2 operator* (float scalor, const Vector2& vec)
	{
	...
	Vector2& operator += (const Vector2& vec)
	{
		this->x += vec.x;
		this->y += vec.y;
		return *this; //returnした値を直接参照している状態
	}
	...
	friend const Vector2 Normalize(Vector2 vec)
	{
